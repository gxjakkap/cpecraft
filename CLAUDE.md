...
- **`luckperms/LuckPermsService`** — thin wrapper around
  `LuckPermsProvider.get()` / `UserManager.modifyUser(...)`. Catches
  `IllegalStateException` and no-ops with a logged warning if LuckPerms
  isn't installed (it's a `compileOnly` dependency — the LuckPerms Fabric
  plugin, a separate admin-installed mod, provides the runtime
  implementation).
...
- **`welcome/WelcomeEventListeners`** — separate `ServerPlayConnectionEvents.JOIN`
  ...
  uses) and the parsed MOTD to chat.
